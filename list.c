...
int remove_data(list* llist, const void* data, equal_op compare_func, list_op free_func)
{
  ...
  //      printf("find worker in compare func");
...
/** find_occurence
  ...
  */
void* find_occurrence(list* llist, const void* key, equal_op compare_func)
{
  ...
  for ( i=0; i<llist->size; i++) {
    ...
    if (compare_func(key, current->data)) 
        return current->data;
    current = current->next;
  }
